...
from models.networks import subnet
from models.networks import rgb2hsv
import sys
from models.PIQE import piqe
from models.NIQE import niqe
...
class SingleModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.rgb2hsv = rgb2hsv(channel_in = 3, channel_out = 4)
        ...
        if self.isTrain:
            ...
            if self.opt.latent == 1:
                self.latent = networks.Latnentnet(skip=skip, opt=opt)
            if len(opt.gpu_ids) > 0:
                self.latent.cuda(device=opt.gpu_ids[0])
                self.latent = torch.nn.DataParallel(self.latent, opt.gpu_ids)
            self.latent.apply(networks.weights_init)
            
            self.netD_A = networks.define_D(opt.output_nc, opt.ndf,
                                            opt.which_model_netD,
                                            opt.n_layers_D, opt.norm, use_sigmoid, self.gpu_ids, False)
            self.netD_latent = networks.define_D(opt.output_nc, opt.ndf,
                                            opt.which_model_netD,
                                            opt.n_layers_D, opt.norm, use_sigmoid, self.gpu_ids, False)
            if self.opt.patchD:
        ...
        if self.isTrain:
            ...
            self.optimizer_D_latent = torch.optim.Adam(self.netD_latent.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999))
            if self.opt.patchD:
        ...
        if self.isTrain:
            if self.opt.latent==1:
                networks.print_network(self.latent)
    ...
    def test(self):
        ...
        if self.opt.which_model_netG == 'INV_HSV':
                self.latent_real_A = self.netG_A(self.real_A)
                self.fake_B = self.netG_A(self.latent_real_A, rev=True)
        else:
            if self.opt.skip == 1:
    ...
    def backward_D_latent(self):
        ...
        real_latent = self.real_latent
        self.loss_D_latent = self.backward_D_basic(self.netD_A, real_latent, pred_latent, True)
        self.loss_D_latent.backward()
    ...
    def forward(self):
        ...
        if self.opt.which_model_netG == 'INV_HSV':
                self.latent_real_A = self.netG_A(self.real_A)
                self.latent_real_b = self.rgb2hsv(self.real_B)
                #debug()
                self.latent_real_b = self.latent_real_b.cuda(device = self.opt.gpu_ids[0])                
                self.y_ = torch.cat((self.latent_real_b[:,:1,:,:], self.latent_real_A[:, 1:, :, :]), dim=1)              
                self.fake_B = self.netG_A(self.y_, rev=True)
                self.fake_B = self.fake_B.cuda(device = self.opt.gpu_ids[0])
        ...
        if self.opt.latent==1:
            self.pred_latent = self.latent(self.fake_B)
            self.real_latent = self.latent(self.real_B)
        
        self.optimizer_D_latent.zero_grad()
        if self.opt.latent==1:    
            self.backward_D_latent()
        #debug()
        self.opt.patchD_3 =  round(0*self.loss_D_latent.item())
        if self.opt.patchD:
    ...
    def backward_G(self, epoch):
        ...
        fk_B = util.tensor2im(self.fake_B.data)
        self.niqe = niqe(fk_B)
        self.piqe = piqe(fk_B)
        ...
        if self.opt.patchD:
            ...
            if self.opt.hybrid_loss:
                loss_G_A = loss_G_A + self.criterionGAN(pred_fake_patch, True)
            else:
                pred_real_patch = self.netD_P.forward(self.real_patch)
                
                loss_G_A = loss_G_A + (self.criterionGAN(pred_real_patch - torch.mean(pred_fake_patch), False) +
        ...
        if self.opt.patchD_3 > 0:   
            for i in range(self.opt.patchD_3):
                ...
                if self.opt.hybrid_loss:
                    loss_G_A = loss_G_A + self.criterionGAN(pred_fake_patch_1, True)
                else:
                    pred_real_patch_1 = self.netD_P.forward(self.real_patch_1[i])
                    
                    loss_G_A = loss_G_A+ (self.criterionGAN(pred_real_patch_1 - torch.mean(pred_fake_patch_1), False) +
                                        self.criterionGAN(pred_fake_patch_1 - torch.mean(pred_real_patch_1), True)) / 2
                    
            if not self.opt.D_P_times2:
                self.loss_G_A = self.loss_G_A + loss_G_A/float(self.opt.patchD_3 + 1)
            else:
                self.loss_G_A = self.loss_G_A + loss_G_A/float(self.opt.patchD_3 + 1)*2
        else:
            if not self.opt.D_P_times2:
                self.loss_G_A = self.loss_G_A + loss_G_A
            else:
                self.loss_G_A = self.loss_G_A + loss_G_A*2
        ...
        if self.opt.vgg > 0:
            ...
            if self.opt.patch_vgg:
                ...
                if self.opt.patchD_3 > 0:
                    for i in range(self.opt.patchD_3):
                        if not self.opt.IN_vgg:
                            loss_vgg_patch = loss_vgg_patch + self.vgg_loss.compute_vgg_loss(self.vgg, 
                                self.fake_patch_1[i], self.input_patch_1[i]) * self.opt.vgg
                        else:
                            loss_vgg_patch = loss_vgg_patch+self.vgg_patch_loss.compute_vgg_loss(self.vgg, 
                                self.fake_patch_1[i], self.input_patch_1[i]) * self.opt.vgg
                    self.loss_vgg_b = self.loss_vgg_b+loss_vgg_patch/float(self.opt.patchD_3 + 1)
                else:
                    self.loss_vgg_b = self.loss_vgg_b + loss_vgg_patch
            self.loss_G = self.loss_G_A + self.loss_vgg_b*vgg_w + self.piqe + self.niqe
            #debug()
        elif self.opt.fcn > 0:
            ...
            self.loss_G = self.loss_G_A + self.loss_fcn_b*vgg_w + self.piqe + self.niqe 
    ...
    def get_current_errors(self, epoch):
        ...
        if self.opt.latent==1:    
            D_latent = self.loss_D_latent.item()
        piqe = self.piqe.item()
        niqe = self.niqe.item()
        if self.opt.vgg > 0:
            vgg = self.loss_vgg_b.item()/self.opt.vgg if self.opt.vgg > 0 else 0
            if self.opt.latent==1:
                return OrderedDict([('D_A', D_A), ('G_A', G_A), ("vgg", vgg), ("D_P", D_P),("D_latent", D_latent),("piqe", piqe),("niqe", niqe)])
            return OrderedDict([('D_A', D_A), ('G_A', G_A), ("vgg", vgg), ("D_P", D_P),("piqe", piqe)])
        elif self.opt.fcn > 0:
            fcn = self.loss_fcn_b.item()/self.opt.fcn if self.opt.fcn > 0 else 0
            return OrderedDict([('D_A', D_A), ('G_A', G_A), ("fcn", fcn), ("D_P", D_P),("D_latent", D_latent),("piqe", piqe),("niqe", niqe)])
    ...
    def get_current_visuals(self):
        ...
        real_latent = util.tensor2im(self.real_latent.data)
        pred_latent = util.tensor2im(self.pred_latent.data)
        if self.opt.skip > 0:
            latent_real_A = util.tensor2im(self.latent_real_A.data)
            latent_show = util.latent2im(self.latent_real_A.data)
            if self.opt.patchD:
                ...
                if self.opt.patch_vgg:
                    ...
                    if not self.opt.self_attention:
                        return OrderedDict([('real_A', real_A), ('fake_B', fake_B), ('latent_real_A', latent_real_A),
                                ...
                                ('fake_patch', fake_patch), ('input_patch', input_patch),('real_latent', real_latent),('pred_latent', pred_latent)])
                    else:
                        self_attention = util.atten2im(self.real_A_gray.data)
                        return OrderedDict([('real_A', real_A), ('fake_B', fake_B), ('latent_real_A', latent_real_A),
                                ('latent_show', latent_show), ('real_B', real_B), ('real_patch', real_patch),
                                ('fake_patch', fake_patch), ('input_patch', input_patch), ('self_attention', self_attention),('real_latent', real_latent),('pred_latent', pred_latent)])
                else:
                    if not self.opt.self_attention:
                        return OrderedDict([('real_A', real_A), ('fake_B', fake_B), ('latent_real_A', latent_real_A),
                                ('latent_show', latent_show), ('real_B', real_B), ('real_patch', real_patch),
                                ('fake_patch', fake_patch),('real_latent', real_latent),('pred_latent', pred_latent)])
                    else:
                        self_attention = util.atten2im(self.real_A_gray.data)
                        return OrderedDict([('real_A', real_A), ('fake_B', fake_B), ('latent_real_A', latent_real_A),
                                ('latent_show', latent_show), ('real_B', real_B), ('real_patch', real_patch),
                                ('fake_patch', fake_patch), ('self_attention', self_attention),('real_latent', real_latent),('pred_latent', pred_latent)])
            else:
                if not self.opt.self_attention:
                    return OrderedDict([('real_A', real_A), ('fake_B', fake_B), ('latent_real_A', latent_real_A),
                                ('latent_show', latent_show), ('real_B', real_B),('real_latent', real_latent),('pred_latent', pred_latent)])
                else:
                    self_attention = util.atten2im(self.real_A_gray.data)
                    return OrderedDict([('real_A', real_A), ('fake_B', fake_B), ('real_B', real_B),
                                    ('latent_real_A', latent_real_A), ('latent_show', latent_show),
                                    ('self_attention', self_attention),('real_latent', real_latent),('pred_latent', pred_latent)])
        else:
            if not self.opt.self_attention:
                return OrderedDict([('real_A', real_A), ('fake_B', fake_B), ('real_B', real_B),('real_latent', real_latent),('pred_latent', pred_latent)])
            else:
                self_attention = util.atten2im(self.real_A_gray.data)
                return OrderedDict([('real_A', real_A), ('fake_B', fake_B), ('real_B', real_B),
                                    ('self_attention', self_attention),('real_latent', real_latent),('pred_latent', pred_latent)])
    ...
    def save(self, label):
        ...
        self.save_network(self.latent, 'D_A', label, self.gpu_ids)
        if self.opt.latent:
            self.save_network(self.netD_latent, 'D_A', label, self.gpu_ids)
        if self.opt.patchD:
